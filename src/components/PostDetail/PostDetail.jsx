...
const PostDetail = ({ postDetail }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const serverResponse = await axios.post(`https://captiverse-app.up.railway.app/api/comment/content/${postDetail.id}`, commentData, {
        headers: {
  ...
  const handleDeletePost = async () => {
    try {
      await axios.delete(`https://captiverse-app.up.railway.app/api/content/${postDetail.id}`, {
        headers: {
          access_token: localStorage.getItem("access_token"),
        },
      });
  ...
  const handleDeleteComment = async (commentId) => {
    try {
      await axios.delete(`https://captiverse-app.up.railway.app/api/comment/${commentId}`, {
        headers: {
          access_token: localStorage.getItem("access_token"),
        },
      });
  ...
  return (
    <div className="container mx-auto mt-8 bg-dark-2 rounded-lg overflow-hidden shadow-md">
      {postDetail && (
        <div className="max-w-screen-xl mx-auto md:flex justify-center">
          <div className="md:w-3/5 relative">
            <div className="aspect-w-16 aspect-h-10 rounded-lg overflow-hidden transform">
              <img className="w-full h-full object-cover scale-95" src={postDetail.photo} alt="Post Image" />
            </div>
          </div>
          <div className="md:w-2/5 p-6 flex flex-col">
            ...
            <div className="flex items-center mb-2 mt-auto">
              <img className="w-8 h-8 cursor-pointer mr-3" src={liked ? "/assets/liked.svg" : "/assets/like.svg"} alt="Like Icon" onClick={() => handleLike(postDetail.id)} />
